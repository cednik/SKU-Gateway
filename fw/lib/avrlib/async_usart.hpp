...
#include "null_pin.hpp"

#include <Stream.h>

namespace avrlib {

template <typename Usart, int RxBufferSize, int TxBufferSize, typename Bootseq = nobootseq, typename Overflow = uint32_t, typename Dir = null_pin<0> >
class async_usart: public Stream
{
public:
	...
	async_usart()
		 :m_overflow(0), m_transmitted(true)
	{
	}
	...
	typename buffer<value_type, RxBufferSize>::index_type available() const
	{
	...
	int peek()  // because of compatibility with Arduino Stream
	{
		return m_rx_buffer.empty() ? -1 : m_rx_buffer.top();
	}

	//value_type read()
	int read() // because of compatibility with Arduino Stream
	{
	...
	size_t write(value_type v) // because of compatibility with Arduino Stream
	{
		m_transmitted = false;
		if(TxBufferSize != 0)
		{
	...
	int available() // because of compatibility with Arduino Stream
	{
		return m_rx_buffer.size();
	}
